      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="backdrop-blur rounded-2xl shadow-xl p-8 border border-gray-200 card-hover">
          <div class="flex items-center justify-between mb-6">
            ...
            <span class="bg-green-100 text-green-800 text-sm font-medium px-3 py-1 rounded-full">{{ familyMembers.length }} человек</span>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div v-else v-for="member in familyMembers" :key="member.uuid" class="p-4 bg-white rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
              <!-- Member content -->
              ...
              <div v-if="member.familyDto" class="mt-3 pt-3 border-t border-gray-100">
                <span class="text-xs text-gray-500">Семья:</span>
                <span class="text-sm text-gray-700 ml-2">{{ member.familyDto.familyName }}</span>
              </div>
              <div class="flex gap-2 mt-3">
                <button @click="editMember(member.uuid)" class="text-xs bg-green-500 text-white px-2 py-1 rounded">
                  Редактировать
                </button>
                <button @click="deleteMember(member.uuid)" class="text-xs bg-red-500 text-white px-2 py-1 rounded">
                  Удалить
                </button>
              </div>
            </div>
